...
把合作信息推到公司现用的飞书多维表格。**这一步需要这台机器能访问外网**，
但同步失败不影响商务干活 —— 断网时照样能建档，恢复后队列自动补推。

1. 飞书开发者后台建一个**企业自建应用**，权限勾上多维表格读写
2. **打开那张多维表格**，右上角「…」→「更多」→「添加文档应用」，把应用加进去
3. 在那张表里加一列（比如「系统ID」，文本类型）
4. 系统里：设置 → 飞书同步 → 填 App ID / Secret / 表格链接 → 测试连接 → 选表 → 映射列

> 第 2 步最容易漏。**只在开发者后台申请权限是不够的**，
> 少了这一步接口一律返回 91403，而飞书的原始错误只说 `Forbidden`。
> 系统已经把这个码翻译成了可操作的提示，看到「应用没被加进这张表」就是指这里。

**被同步的那几列在飞书里手工改会被下次推送覆盖** —— 单向推送，
飞书那边当只读看板用。
...
| 飞书表不更新 | 设置 → 飞书同步 看队列和错误；多半是应用没被加进那张表 |
