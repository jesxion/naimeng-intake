...
/**
 * 把当前 db.json 复制一份到 data/backups/，只保留最近 KEEP_BACKUPS 份。
 *
 * 存在的理由很具体：这个库是整文件覆写的，没有文件锁。误开两个进程、
 * 或者跑一段脚本时数据目录指错，都会静默把真实数据冲掉，且没有任何回滚点。
 * 备份便宜（几十 KB），丢数据很贵。
 */
export async function backupNow(reason = 'startup') {
  return backupSync(reason);
}

/* 同步内核。load() 在解析失败时要立刻留档，而 load() 本身是同步的
   （JSON 阶段它就是同步的，换 Postgres 时整个函数会消失），
   所以备份不能只有异步版本。对外仍然只暴露 async 的 backupNow ——
   换存储时实现改成异步，调用方一行都不用动。 */
function backupSync(reason = 'startup') {
  if (!existsSync(DB_FILE)) return null;
  try {
    mkdirSync(BACKUP_DIR, { recursive: true });
    const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const target = join(BACKUP_DIR, `db-${ts}-${reason}.json`);
    copyFileSync(DB_FILE, target);

    // 文件名里的时间戳是定长的，字典序等于时间序
    const old = readdirSync(BACKUP_DIR).filter((f) => f.startsWith('db-') && f.endsWith('.json')).sort();
...
function load() {
  ...
  try {
    ...
    // 原来这里直接换成空库继续跑 —— 下一次写入就会把损坏但可能还能救的文件彻底覆盖掉。
    // 现在先留一份原件，并且把话说明白，不让它悄悄过去。
    const saved = backupSync('corrupt');
    console.error('');
    console.error('  ⚠ data/db.json 读不出来：' + e.message);
    console.error('  ⚠ 已改用空库继续运行，你现在看到的会是「没有任何数据」。');
    if (saved) console.error('  ⚠ 损坏的原件留在 ' + saved + '，先别动它。');
    console.error('  ⚠ 想恢复就从 data/backups/ 里挑一份改名成 db.json。');
    console.error('');
    cache = structuredClone(EMPTY);
  }
  migrate();
...
export async function listUsers() { return load().users; }
export async function getUser(id) { return load().users.find((u) => u.id === id) || null; }

export async function getSettings() { return loadSettings(); }
...
export async function currentUser(headerId) {
  const db = load();
  // 顺序很重要：浏览器自报的身份优先于全局设置。
  // 反过来写的话，settings.user 是单例，四个商务同时用就会共享一个身份 ——
  // 任何一人在设置里改名，其他人的归属和数据范围会跟着一起变。
  if (headerId) {
...
export async function createCreator(payload, userId) {
  ...
  await addAccounts(id, payload.accounts || [], payload.otherAccounts || []);
  flush();
  return creator;
}
...
export async function addPackage(collaborationId, { carrier, trackingNo, shippedAt = null, source = 'manual' }) {
  const db = load();
...
export async function getFulfillment(id) {
  ...
  return { ...f, account: db.accounts.find((a) => a.id === f.accountId) || null,
    collaboration: await getCollaboration(f.collaborationId) };
}
...
export async function getDraft(id) { return load().drafts.find((d) => d.id === id) || null; }
...
export async function createJob({ ownerUserId, rawText, kind = 'intake', imageBase64 = null }) {
  const db = load();
...
export async function getJob(id) { return load().jobs.find((j) => j.id === id) || null; }
...
export async function runningCount() { return load().jobs.filter((j) => j.status === 'running').length; }
