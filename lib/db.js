...
export async function getUser(id) { ensure(); return store.get('users', id); }
...
export async function saveSettings(patch = {}) {
  ...
  if (patch.user) {
    ...
    if (name) {
      let u = store.all('users').find((x) => x.name === name);
      if (!u) u = { id: nextId('u'), name, role: s.user.role, phone: s.user.phone };
  ...
  if (patch.feishu) {
    ...
    const { appSecret, clearSecret, mapping, ...rest } = patch.feishu;
