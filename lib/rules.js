...
/**
 * 把用户填的地址归一成纯 origin（协议+主机+端口）。认不出来返回空串。
 *
 * CORS 比对的是**源**，不是 URL。存了带路径的值只会在比对时永远不匹配，
 * 而那个现象看起来像「白名单配了但没生效」—— 极难往「多了个斜杠」上想。
 * 所以在保存的边界上归一，和 appToken 那次是同一个道理。
 */
...
export function normalize(extracted) {
  ...
  const form = {
    ...
    /* 合作类型。模型给的只是建议，商务在界面上能改 ——
       但之前这个值一路丢到了后端拿不到的地方，所有合作都落成「寄样合作」。 */
    type: COLLAB_TYPES.includes(e.cooperation_type) ? e.cooperation_type : '寄样合作',
    /* 带货方式。和 type 是同一个漏：模型抽得到（提示词里有，还有正则兜底），
       但 normalize 不输出、路由不传，于是 `collaborations.salesChannel`
       永远是空字符串，详情里那一行永远不显示。 */
    salesChannel: cleanValue(e.sales_channel?.v ?? e.sales_channel),
    items: [],          // 商务手选，微信资料里没有
 ...
 * 合作类型。
 *
 * 「直播定向」和「不寄样合作」都不寄实物 —— 前者是给直播间开定向，
 * 后者是达人手里已经有货或纯投流。两者对系统的意义完全一样：
 * 不需要产品、不需要收件信息、不该催快递单号。
 */
export const COLLAB_TYPES = ['寄样合作', '不寄样合作', '直播定向'];

const NON_SAMPLE = new Set(['不寄样合作', '直播定向']);

/**
 * 这种合作要不要寄实物。
 *
 * **所有「要不要收件地址 / 要不要产品 / 要不要催单号」的判断都走这一个函数。**
 * 散在各处写 `type === '直播定向'` 的话，以后再加一种不寄样的类型，
 * 就得把每一处都找出来改 —— 找漏一处的表现是「某一步莫名其妙拦住了」，
 * 而没人会想到是类型判断漏了。
 */
export const needsSample = (type) => !NON_SAMPLE.has(type);
...
export function validateForAction(form, action) {
  ...
  /* 不寄样的合作没有「提交寄样」这一步，产品和收件信息也就无从必填。
     注意判断的是类型，不是「填没填」—— 按「填没填」来放行的话，
     寄样合作漏填地址也会被静悄悄放过去，那正是这套校验要防的事。 */
  if (action === 'submitSample' && needsSample(f.type)) {
    const r = f.recipient || {};
  ...
  if (action === 'submitSample' && needsSample(f.type)
      && (f.sampleCost === null || f.sampleCost === undefined || f.sampleCost === '')) {
    soft.push('未填寄样费用');
  }
...
export function sanitizeForStore(form) {
  ...
  return {
    ...
    /* 类型必须白名单化再存 —— 它决定要不要校验收件信息，
       放任意字符串进来等于给了一条绕过校验的路。 */
    type: COLLAB_TYPES.includes(f.type) ? f.type : '寄样合作',
    salesChannel: cleanValue(f.salesChannel),
...
export function buildTodos({ collaborations = [], drafts = [], jobs = [], followUp = {}, now = Date.now() }) {
  ...
  for (const cb of collaborations) {
    ...
    /* 不寄样的合作本来就没有产品和快递 —— 这两条待办对它是噪音，
       而噪音多了真正该处理的就被淹没了。 */
    const shipping = needsSample(cb.type);

    // 迁移来的老记录缺商品/费用
    if (shipping && (cb.items || []).length === 0) {
    ...
    if (shipping && !hasPackage && cb.status === '待寄样') {
