...
{
  "cooperation_type": "寄样合作" / "不寄样合作" / "直播定向",
...
12. 若文本表明这是直播间账号，或诉求是「开定向」「定向高佣」，把 cooperation_type 设为 "直播定向"。
13. 若文本明确说明不需要寄样（达人手里已有货、只谈投流、只要开白），设为 "不寄样合作"。
    "直播定向" 和 "不寄样合作" 都不寄实物，此时 recipient 各项可以全为 null，也不要编造地址。
14. 收件人可能是单字（"孙"）、称谓（"李先生"、"王女士"）、网名（"萝卜"、"旺仔"）或店铺名（"示例童装馆"）。不要因为不像姓名就丢弃。
15. 姓名、手机号、地址常常粘连在一行且顺序不定，例如「张女士13800138001，示例省示例市…」或「…示范小区1栋22号202室 李某某13800138002」。要正确拆开。
...
export async function extract(rawText, previous = null) {
  ...
  /* 必须 await。agentReady 是 async，`!agentReady()` 里 Promise 恒为 truthy，
     取反永远是 false —— 这个 mock 分支曾经是死代码，未配置模型时
     不走降级而是真去调接口，然后以「无法连接」失败。
     和之前 agentConfig 漏 await 是同一个坑：那次修了 server.js 的调用点，
     漏了 agent.js 内部这两处。 */
  if (!await agentReady()) {
...
export function mockExtract(text) {
  const out = {
    cooperation_type: /直播间|开定向|定向高佣/.test(text) ? '直播定向'
      : /不寄样|不用寄|无需寄样|已有货|自己有货|只投流|只开白/.test(text) ? '不寄样合作'
      : '寄样合作',
