 ...
 */
export function agentConfig(which = 'model') {
...
const ready = (which) => {
  const c = agentConfig(which);
  return Boolean(c.baseUrl && c.model && c.apiKey);
};

export function agentReady() { return ready('model'); }

/** 未配置视觉模型时，前端禁用截图上传 */
export function visionReady() { return ready('vision'); }
...
export async function testConnection(which = 'model', override = null) {
  const cfg = override ? { ...agentConfig(which), ...override } : agentConfig(which);
