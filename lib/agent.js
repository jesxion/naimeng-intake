...
export function agentConfig() {
  const s = db.getSettings().model || {};
  return {
    provider: s.provider || '',
    baseUrl: String(s.baseUrl || process.env.LLM_BASE_URL || '').replace(/\/+$/, ''),
    model: s.model || process.env.LLM_MODEL || '',
    apiKey: s.apiKey || process.env.LLM_API_KEY || '',
    apiStyle: s.apiStyle === 'responses' ? 'responses' : 'chat',
    timeout: Number(s.timeoutMs || process.env.LLM_TIMEOUT_MS || 60000),
    concurrency: Number(s.concurrency || process.env.LLM_CONCURRENCY || 3),
  };
}
...
export async function testConnection(override = null) {
  const cfg = override ? { ...agentConfig(), ...override } : agentConfig();
  ...
  try {
    ...
    return {
      ok: false,
      elapsedMs: Date.now() - started,
      error: e.name === 'AbortError' ? '连接超时（20s）' : e.message,
    };
  } finally {
    clearTimeout(timer);
  }
