...
function explain(code, msg) {
  const map = {
    ...
    /* 读能成、写不成，走到这里。和 91403 是两回事：
       91403 是压根进不来，这个是进来了但只能看。 */
    1254302: '只有读权限，写不了。按这三条依次查：\n'
           + '    1) 开发者后台「权限管理」勾的是不是只读版 ——\n'
           + '       需要「多维表格 编辑」（bitable:app），不是 bitable:app:readonly；\n'
           + '       **改完必须发布新版本并等管理员通过，否则不生效**\n'
           + '    2) 「添加文档应用」时给的权限是不是「可阅读」，要改成「可编辑」\n'
           + '    3) 这张多维表格如果开了「高级权限」，得在高级权限里\n'
           + '       单独给这个应用编辑角色 —— 文档级的授权不会自动继承下去',
    1254303: '没有这张表的写权限，同 1254302 的三条排查',
  };
  const hint = map[code];
...
async function callRaw(method, path, token, body) {
  ...
  try {
    ...
    try { return JSON.parse(text); }
      ...
      /* 带上完整 URL。这次踩到的 404 是因为 app_token 存成了整条分享链接，
         被直接拼进了路径 —— 不打印 URL 的话，「不是 JSON」这句话
         对定位毫无帮助，只能靠猜。 */
      const hint = res.status === 404
        ? '。404 说明这个接口路径不存在，多半是 app_token 或 table_id 不对（比如误存成了整条分享链接）'
        : '';
      throw new Error(`飞书返回的不是 JSON（HTTP ${res.status}）${hint}\n  请求地址：${BASE}${path}\n  响应开头：${text.slice(0, 120)}`);
    }
  } catch (e) {
...
async function call(cfg, method, path, body) {
  /* 路径里出现 :// 或空段，说明某个 id 是空的或塞进了整条 URL。
     与其发出去换一个 404 HTML 页，不如就地说清楚是哪个参数坏了。 */
  if (path.includes('://') || /\/\//.test(path.replace(/^\//, ''))) {
    throw new Error(`接口路径拼错了，请检查多维表格链接和数据表是否填对：${path.slice(0, 160)}`);
  }
 ...
 * 列出这个 Base 里的数据表。
 *
 * 注意「数据表」和侧边栏里看到的东西不是一回事：
 * **仪表盘、表单、看板都排在同一列里，但它们不是数据表，这个接口不会返回。**
 * 视图（表格视图、看板视图…）挂在表下面，也不会单独出现。
 * 所以侧边栏数得出 4 个、接口只回 3 个，多半是正常的。
 *
 * 真正会导致「该有的表没回来」的情况只有一种：
 * 那张表单独开了高级权限，而应用不在它的协作者里。
 *
 * 分页：单页最多 100，超过要靠 page_token 翻。四张表用不到，
 * 但漏掉分页是那种「今天好好的、明天表多了就静默少数据」的坑。
 */
export async function listTables(cfg, appToken) {
  ...
  for (let guard = 0; guard < 50; guard++) {
    ...
    const d = await call(cfg, 'GET', `/bitable/v1/apps/${appToken}/tables?${q}`);
    for (const t of d.items || []) out.push({ tableId: t.table_id, name: t.name });
 ...
 * 在表里新建一列。
 *
 * 存在的理由：同步必须有一列存本系统的合作 ID，否则每次推送都会新建、
 * 飞书表里堆重复行。让用户自己去飞书加列是可以的，但那是一步很容易卡住的
 * 手工操作 —— 加错类型（比如选了「自动编号」）还会导致这一列根本不可写，
 * 而界面上只会表现为「下拉里没有这一列」，用户根本不知道为什么。
 *
 * 所以给一个按钮直接建，类型由我们定死为多行文本。
 */
export async function createField(cfg, appToken, tableId, fieldName, type = 1) {
  const d = await call(cfg, 'POST', `/bitable/v1/apps/${appToken}/tables/${tableId}/fields`, {
    field_name: fieldName,
    type,
  });
  return { fieldId: d.field?.field_id || null, name: d.field?.field_name || fieldName };
}
 ...
 * 删一条记录。
 *
 * 这是我们唯一会删飞书数据的地方，用途只有一个：合作里删掉一款产品后，
 * 清掉对应的那一行。**只删 sync_links 里记着的 record_id** ——
 * 也就是只删我们自己建过的行，手工加的行绝不碰。
 *
 * 记录早就不在了（1254005）当成功处理：目的已经达到了。
 */
export async function deleteRecord(cfg, appToken, tableId, recordId) {
  try {
    await call(cfg, 'DELETE', `/bitable/v1/apps/${appToken}/tables/${tableId}/records/${recordId}`);
  } catch (e) {
    if (e.code !== 1254005) throw e;
  }
  return true;
}
...
export async function testConnection(cfg, appToken, tableId = null) {
  ...
  try {
    ...
    /* 把表名和 table_id 一起报出来。
       用户在侧边栏数到的数量常常和这里对不上 —— 仪表盘、表单、看板
       都排在同一列但不是数据表。列清楚了才能自己比对是哪一项没回来。 */
    steps.push({
      ok: true,
      step: '访问多维表格',
      detail: `返回 ${tables.length} 张数据表：${tables.map((t) => t.name).join('、') || '（无）'}`,
      tables: tables.map((t) => ({ name: t.name, tableId: t.tableId })),
      note: '仪表盘 / 表单 / 看板不算数据表，不会出现在这里',
    });
  } catch (e) {
  ...
  if (tableId) {
    ...
    try {
      ...
      /* 上面全是读操作。读得通不等于写得进 —— 应用可能只有只读权限，
         而那要等第一次真正推送才会暴露（1254302）。
         「测试连接」说成功、同步却一直失败，是最难受的一种失败方式，
         所以这里把话说明白，不冒充已经验证过写入。 */
      steps.push({
        ok: true, step: '写入权限', warn: true,
        detail: '未验证 —— 以上都是读操作。写入要等第一次推送才知道；'
              + '若报 1254302，是应用只有只读权限',
      });
